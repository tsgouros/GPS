...
folders = {pwd, [pwd '/parameters']};
folders = inputdlg({'Main GPS folder', 'Parameters Folder'}, 'Select where you would like to store GPS data', [1 50], folders);
...
exfolders = {fshome, '$FREESURFER_HOME/fsfast', '$FREESURFER_HOME/mni', mneroot, matlabroot, studydir};
exfolders = inputdlg({'Freesurfer home folder', 'Fsfast Folder', 'MNI Folder', 'MNE Folder', 'Home folder for Matlab app', 'Where will study data live?'}, 'Select where you keep the important GPS tools and study data', [1 70], exfolders);

% Citation tag
citation = sprintf('##\n## Author: Tom Sgouros (tom@sgouros.com)\n##\n## Changelog:\n## %s Automatically generated by gps_init by user %s\n##', date, user);

% Change the directory pointing program
fid = fopen('gps_init.bash', 'w');
...
fprintf(fid, 'setenv MATLAB_ROOT %s\n', exfolders{5});
