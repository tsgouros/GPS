function varargout = gpsa_granger_sigtest2(varargin)
...
% 2013.05.08 - GPS1.8 gpsa_granger_sigcompile.m Created
...
if(~isempty(strfind(operation, 'c')))
    ...
    state.function = 'gpsa_granger_sigcompile';
    tbegin = tic;
    ...
    % Granger Activation
    subtype = 'Endpoint';
    filename = sprintf('%s/results/%s_%s_CvI_sigmetrics.mat', study.granger.dir, study.name, subtype);
    data = load(filename);
    
    %% Do GCI comparisons
    
    N_rois = size(data.A.results, 1);
    countA = zeros(N_rois, N_rois);
    countB = zeros(N_rois, N_rois);
    countdiff = zeros(N_rois, N_rois);
    p_values = zeros(N_rois, N_rois);
    time_window = 201:401;
    
    for i_src = 1:N_rois % Sources
        for i_snk = 1:N_rois % Sinks
            as = squeeze(data.A.p_values(i_snk, i_src, time_window) >= 0.95);
            countA(i_snk, i_src) = sum(as);
            
            bs = squeeze(data.B.p_values(i_snk, i_src, time_window) >= 0.95);
            countB(i_snk, i_src) = sum(bs);
            
            countdiff(i_snk, i_src) = countA(i_snk, i_src) - countB(i_snk, i_src);
            p_values(i_snk, i_src) = gps_countdistinct(as', bs');
        end
    end
    
    data.p1.countA = countA;
    data.p1.countB = countB;
    data.p1.countdiff = countdiff;
    data.p1.p_values = p_values;
    data.p1.time_period = '200to400';
    ...
    data.p2.countA = countA;
    data.p2.countB = countB;
    data.p2.countdiff = countdiff;
    data.p2.p_values = p_values;
    data.p2.time_period = '050to250';
