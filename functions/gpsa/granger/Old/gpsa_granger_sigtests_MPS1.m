function varargout = gpsa_granger_sigtests_MPS1(varargin)
...
if(~isempty(strfind(operation, 'c')))
    ...
    % Determine filenames
    %Must select an SS condition first to then compare to SO
...
%         subtype = state.subset(3:end)
%     
%         state2.subset = ['so' subtype];
    subtype = state.condition(1:end-1);
    state2.condition = [subtype 'R'];
    ...
    % Set the opposition names
    ops = {'ss', 'so'};
    opps = {'I', 'R'};
    Opps = {'Constrained', 'Reversible'};
    oppv = 'IvR';
    %% Get P-Values
    N_comp = 2000;
    ...
    g1.perc95 = ones(size(g1.results))*10;
    ...
    g2.perc95 = ones(size(g2.results))*10;
...
%         subtype = subset.name(1:find(condition.name == '_', 1, 'first')-1);
%     else
%         subtype = 'Ambiguous';
%     end
    
    % Time settings
    tstart = str2double(state.subset(1:3));
    tend = str2double(state.subset(6:8));
    times = tstart:tend;
    N_time = length(times);
    time_index = (tstart:tend) + 1;
    subset = ['/' state.subset];
    
    % Select Certain ROIs
    srcs_names = {'L-ParsOper', 'L-ParsTri', 'L-ParsOrb', 'L-MTG', 'L-STG'};
    snks_names = {'L-ParsOper', 'L-ParsTri', 'L-ParsOrb', 'L-MTG', 'L-STG'};
    
    % Get ROI names
    rois = sprintf('%s/rois/%s%s/*.label', study.granger.dir, condition.cortex.roiset, subset);
    rois = dir(rois);
    rois = {rois.name};
    N_rois = length(rois);
...
%     srcs_index = 1:N_rois;
    ...
    points = sprintf('%s/rois/%s%s/%s/%s_rois.mat', study.granger.dir, condition.cortex.roiset, subset, state.subject, state.subject);
    points = load(points);
    points = points.rois;
...
%                 options.labeltext{i_roi} = sprintf('%s (%d)', rois{i_roi}, spikes(i_roi));
%             end
%             
%             gps_brain_draw(brain, options);
%             
%             frame = getframe(gcf);
%             filename = sprintf('%s/1_bubble/%s_%s_%s_granger_DLT_%s_influencers_cortex_count_multicolor.png', folder, study.name, subtype, spectype, rois{i_snk_inrois});
...
%                     case 'L-MTG'
%                         roicolor = [0 .9 0];
                    case {'L-MTG', 'L-MTG_1', 'L-MTG_2', 'L-MTG_3', 'L-MTG_4'}
                        roicolor = [0 .9 0];
                    case {'L-ParsOper', 'L-ParsTri', 'L-ParsOrb', 'L-ParsOper_1', 'L-ParsTri_1', 'L-ParsOrb_1', 'L-ParsOper_2', 'L-ParsTri_2', 'L-ParsOrb_2', 'L-ParsOper_3', 'L-ParsTri_3', 'L-ParsOrb_3', 'L-ParsOper_4', 'L-ParsTri_4', 'L-ParsOrb_4', 'L-ParsOper_5', 'L-ParsTri_5', 'L-ParsOrb_5'}
                        roicolor = [.9 0 0];
%                     case 'L-SMG'
%                         roicolor = [.9 0 0];
%                     case 'L-STG'
...
%         [h, p] = ttest(x(sel) - y(sel));
        ...
        % Labels
        spikes = sum(draw.values >= draw.criteria, 2);
        draw.labels = {sprintf('%s %s (%d)', opps{1}, comparisons{i_comp}, spikes(1)),...
            sprintf('%s %s (%d)', opps{2}, comparisons{i_comp}, spikes(2))};
...
%         draw.colors_line = zeros(2, 3);
        ...
        % Figure Settings
        draw.legend = 'Northeast'; %perhaps change to be outside southeast
        draw.N_axes = 2;
        draw.fig = 1;
        draw.flag_pvals = 0;
        draw.flag_sigspikes = 1;
...
%         end
        ...
        for flag_decor = 0:flag_decor_do
            try
                ...
                scatter(xr, yr)
                xlabel([Opps{1} ' P Values'])
                ylabel([Opps{2} ' P Values'])
                ...
                if(x_greater)
                    titlestr2 = sprintf('%s %s %s (%d) > %s (%d), diff = %d', subtype, comparisons{i_comp}, Opps{1}, sum(xs), Opps{2}, sum(ys), sum(xs) - sum(ys));
                else
                    titlestr2 = sprintf('%s %s %s (%d) > %s (%d), diff = %d', subtype, comparisons{i_comp}, Opps{2}, sum(ys), Opps{1}, sum(xs), sum(ys) - sum(xs));
                end
...
%         
%         frame = getframe(5);
%         filename = sprintf('%s/4_sigcort/%s_%s_%s_granger_DLT_%s_influencers_cortex_pvaldiff_color.png', folder, study.name, subtype, oppv, rois{i_snk_inrois});
%         imwrite(frame.cdata, filename, 'png');
...
%         
%         frame = getframe(5);
%         filename = sprintf('%s/4_sigcort/%s_%s_%s_granger_DLT_%s_influencers_cortex_pvaldiff_size.png', folder, study.name, subtype, oppv, rois{i_snk_inrois});
%         imwrite(frame.cdata, filename, 'png');
        ...
        figure(5)
        frame = getframe(5);
        filename = sprintf('%s/4_sigcort/%s_%s_%s_granger_DLT_%s_influencers_cortex_countdiff_size.png', folder, study.name, subtype, oppv, rois{i_snk_inrois});
        imwrite(frame.cdata, filename, 'png');
    ...
    for i_comp = 1:N_comp
        ...
        fprintf(fid, '%20s\t%s% 3d %s %s% 3d\tp = %0.3f\tA1 = %.2f\tA2 = %.2f\n', comparisons{i_comp}, ops{1}, s1, sc, ops{2}, s2, p, a1, a2);
    end
    fprintf(fid, '\n%s %s %s > %s Significantly\n', study.name, subtype, Opps{1}, Opps{2});
    for i_comp = 1:N_comp
        s1 = all_spikes(comp_isrcsnks(i_comp, 2), comp_isrcsnks(i_comp, 1), 1);
        s2 = all_spikes(comp_isrcsnks(i_comp, 2), comp_isrcsnks(i_comp, 1), 2);
        sc = '<' + 2 * (s1 > s2) + (s1 == s2);
        ...
        p = all_p(comp_isrcsnks(i_comp, 2), comp_isrcsnks(i_comp, 1));
        if(p <= 0.05 && sc == '>')
            fprintf(fid, '%20s\t%s% 3d %s %s% 3d\tp = %0.3f\tAC = %.2f\tAI = %.2f\n', comparisons{i_comp}, ops{1}, s1, sc, ops{2}, s2, p, a1, a2);
        end
    end
    fprintf(fid, '\n%s %s %s > %s Significantly\n', study.name, subtype, Opps{2}, Opps{1});
    for i_comp = 1:N_comp
        s1 = all_spikes(comp_isrcsnks(i_comp, 2), comp_isrcsnks(i_comp, 1), 1);
        s2 = all_spikes(comp_isrcsnks(i_comp, 2), comp_isrcsnks(i_comp, 1), 2);
        sc = '<' + 2 * (s1 > s2) + (s1 == s2);
