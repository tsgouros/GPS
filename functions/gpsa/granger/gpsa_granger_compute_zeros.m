function varargout = gpsa_granger_compute_zeros(varargin)
...
%fprintf('operation=%s, executing granger_compute_zeros anyway', operation)
if(~isempty(strfind(operation, 'c')))
    
    study = gpsa_parameter(state, state.study);
    condition = gpsa_parameter(state, state.condition);
    state.function = 'gpsa_granger_compute_zero';
    ...
    elseif(isfield(state, 'grangerstream') && strcmp(state.grangerstream, 'seth'))
        ...
        % Initial Kalman
        [sspace, W_all, residual] = gps_kalman_zeros(datafile.data, model_order, pred_adapt);
        
        % Granger
        granger_results = gps_granger_zeros(datafile.data, model_order, pred_adapt);
    end
