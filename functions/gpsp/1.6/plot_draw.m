function plot_draw(GPSP_vars)
% Draws the granger data
%
% Author: Conrad Nied
%
...
% Output: 
%
% Date Created: 2012.07.05 from granger_plot_draw
% Last Modified: 2012.08.07
% 2012.10.11 - Loosely adapted to GPS1.7
% 2013.02.11 - Remakes axis if it was deleted

hObject = GPSP_vars.display_brainaxes;
if(~ishghandle(hObject))
    figure(GPSP_vars.display_brainfig)
    GPSP_vars.display_brainaxes = gca;
    hObject = GPSP_vars.display_brainaxes;
    guidata(GPSP_vars.base_draw, GPSP_vars);
end

% set(GPSP_vars.display_brainfig, 'Units', 'Normalized');
% set(.display_brainfig, 'Position', [0 0 1 1]);
% set(GPSP_vars.display_brainaxes, 'ActivePositionProperty', 'position');
% set(GPSP_vars.display_brainaxes, 'Units', 'Normalized');
% set(GPSP_vars.display_brainaxes, 'Position', [0.05 0.05 0.9 0.9]);

if(~get(GPSP_vars.display_autoredraw, 'Value'))
    guidata(GPSP_vars.base_draw, GPSP_vars);
    return
end
...
cla(GPSP_vars.display_brainaxes);
view(GPSP_vars.display_brainaxes, 0, 90);
legend(GPSP_vars.display_brainaxes, 'off')
...
%             CData = CData + data;
    ...
    if(showside(1))
        ...
        patch('Parent', GPSP_vars.display_brainaxes,...
            'Faces', brain.lface,...
            'Vertices', ll_coords,...
    ...
    % Set View
    view(GPSP_vars.display_brainaxes, 90, 0)

    % Set Lighting
    if(get(GPSP_vars.brain_shading, 'Value'))
        [az, el] = view(GPSP_vars.display_brainaxes);
        [lightx lighty lightz] = sph2cart(az*pi/180, el*pi/180, 100);
        light('Parent', GPSP_vars.display_brainaxes,...
            'Position', [lighty -lightx lightz],...
            'Style', 'infinite');
    end
    
    % Set Axis Limits
    axis(GPSP_vars.display_brainaxes, 'equal','tight');
    xlim(GPSP_vars.display_brainaxes, [-80 80]);
    ylim(GPSP_vars.display_brainaxes, [min([lm_coords(:, 2); rm_coords(:, 2); rl_coords(:, 2); ll_coords(:, 2)]) - 10 ...
        max([lm_coords(:, 2); rm_coords(:, 2); rl_coords(:, 2); ll_coords(:, 2)]) + 10]);
    zlim(GPSP_vars.display_brainaxes, [min([lm_coords(:, 3); rm_coords(:, 3); rl_coords(:, 3); ll_coords(:, 3)]) - 10 ...
