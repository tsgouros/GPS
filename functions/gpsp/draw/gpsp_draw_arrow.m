function gpsp_draw_arrow(A, B, varargin)
...
%
% changelog:
% 2012-08-07 created GPS1.6/plot_arrow from GPS1.5/granger_plot_arrow
% 2013-07-16 Updated to GPS1.8
% 2013-08-12 Can have different recip colors now
...
if(length(color) == 2) % Do we have an interpolating color?
    cA = color(2);
    cB = color(1);
elseif(length(color) == 4) % Two sets of colors
    cA = color(2);
    cB = color(1);
    cA2 = color(4);
    cB2 = color(3);
...
if(exist('cA2', 'var'))
    if(strcmp(style, 'Interior'))
        cC2 = cA2 + (cB2 - cA2) * (width(1) + width(2)) / h / 2;
        cD2 = cB2;
        cE2 = cB2;
        cB2 = cA2 + (cB2 - cA2) * (1 - (width(1) + width(2)) / h / 2);
        cF2 = cE2;
        cG2 = cD2;
        cH2 = cC2;
    else
...
%         FV.facevertexcdata = [cA; cC; cD; cE; cB; cA; cH; cG; cF; cB];
        FV.facevertexcdata = [cA; cC; cD; cE; cB; cA2; cH2; cG2; cF2; cB2];
