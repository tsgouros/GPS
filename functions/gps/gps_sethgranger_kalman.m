function varargout = gps_sethgranger_kalman(data, model_order, pred_adapt, varargin)
% Performs Kalman analysis on the data as a part of granger processing
%
% Author: A. Conrad Nied
%
% Changelog:
% 2012.11.27 - Created, based on GPS1.7/gps_kalman
...
% If there is one, get the suppressed interaction
if(length(varargin) > 1)
    supress = [varargin{1} varargin{2}];
%     supress = supress(1) + (supress(2) - 1) * N_ROIs; % Vectorize
else
    supress = 0;
end
...
%     kalman_gain = state_cov * past_data' / trial_cov;
    ...
    % Surpress
    if(supress(1))
        state_process((0:N_order - 1) * N_ROIs + supress(1), supress(2)) = 0;
    end % If surpressing
