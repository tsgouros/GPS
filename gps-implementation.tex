...
\dataflow{Brain2mat}{\dir{MRI/(subject)/surf},
  ...
  \dir{MRI/avg-subject/brain\_fsaverage.mat}}  Mostly rearranging.
...
\dataflow{Import}{raw fif files somewhere}{\dir{MEG/(subject)/scans}}
  ...
  directories: \dir{events}, \dir{evoked}, \dir{images}, and
  \dir{scans\_filtered}.

  Note that the input data files use a very strict naming convention that looks like  \verb+<+\emph{subject}\verb+>_<+\emph{block}\verb+>_raw.fif+
  where \emph{subject} and \emph{block} are names like ``UAG08'' and
  ``b1''.  The ``blocks'' are essentially sessions of MEG data
  gathering.  If your experiment did not use blocks, you still have to
  assign a block name to each data file, so use ``B1'' or something
  like that.  Example: \verb+UAG08_B1_raw.fif+.
